...
const statusOptions = [
  { value: 'evacuated', label: '✅ Everyone in my unit is out safely', style: 'border-emerald-600 bg-emerald-600 text-white hover:bg-emerald-700 hover:border-emerald-700' },
  { value: 'assistance', label: '⚠️ Some or all of us still need help getting out', style: 'border-amber-500 bg-amber-500 text-slate-900 hover:bg-amber-600 hover:border-amber-600'},
  { value: 'emergency', label: '🚨 Someone is in immediate life-threatening danger', style: 'border-red-600 bg-red-600 text-white hover:bg-red-700 hover:border-red-700' },
]
...
export default function BuildingReportForm({
  ...
  const [selectedFloor, setSelectedFloor] = useState<number | null>(existingReport?.unit.floor ?? null)
  const [selectedUnitId, setSelectedUnitId] = useState<number | null>(existingReport?.unitId ?? null)
  const [residentStatus, setResidentStatus] = useState<ResidentStatus | null>(existingReport?.residentStatus ?? null)
  const [resourceRequests, setResourceRequests] = useState<string[]>(existingReport?.resourceRequests ?? [])
  ...
  function handleOccupantsEvacuatedChange(value: number) {
    ...
    if (value > totalOccupants) { setError('Evacuated occupants cannot exceed total occupants.'); return }
    if (residentStatus === 'evacuated' && value !== totalOccupants) setResidentStatus(null)
    setError(null)
  ...
  async function handleSubmit() {
    ...
    if ((residentStatus === 'assistance' || residentStatus === 'emergency') && resourceRequests.length === 0)
      errors.resources = 'Please select at least one resource'
    if (Object.keys(errors).length > 0) { setFieldErrors(errors); return }
    if (existingReport && selectedUnitId !== existingReport.unitId) {
      setStaleWarning(new Date(existingReport.updatedAt)); return
    }
    await performSubmit()
  }
 ...
 <h3>{building.name}: {building.address}</h3>
      ...
      {staleWarning && (
        <div className="border-l-4 border-amber-400 bg-amber-50 rounded-lg p-4 space-y-3">
          ...
          <p className="text-sm text-amber-700">
            Your original report for Unit {existingReport?.unit.unitNumber} will remain in the system.
          </p>
          <p className="text-sm text-amber-700">Continue?</p>
          <div className="flex gap-3 pt-1">
            <Button type="button" onClick={handleConfirmedSubmit} variant="primary" className="flex-1">Yes, continue</Button>
            <Button type="button" onClick={() => setStaleWarning(null)} variant="outline" className="flex-1">Cancel</Button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Select
              ...
              options={floorUnits.map(unit => ({ value: unit.id, label: `Unit ${unit.unitNumber}` }))}
            />
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-1">Total in unit</label>
          {fieldErrors.totalOccupants && <ErrorMessage message={fieldErrors.totalOccupants} />}
          <Select
        ...
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-1">Already evacuated</label>
          {fieldErrors.evacuated && <ErrorMessage message={fieldErrors.evacuated} />}
          <Select
            ...
            options={occupantOptions.filter(n => n <= (totalOccupants ?? 15)).map(o => ({ value: o, label: `${o}` }))}
          />
        </div>
      </div>
      ...
      {/* Status buttons */}
      <div>
        <label className="block text-sm font-bold text-slate-700 mb-1">Occupant status</label>
        ...
        <p className="text-sm font-medium text-slate-600 mt-2">
          Please select honestly. Accurate status helps responders reach those who need help most.
        </p>
      </div>
 
      {/* Resource requests */}
      <div>
        <label className="block text-sm font-bold text-slate-700 mb-1">Resource requests</label>
        {fieldErrors.resources && <ErrorMessage message={fieldErrors.resources} />}
        <div className="space-y-2">
          {availableResources.map(option => (
            <label
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-lg border-2 cursor-pointer transition-all
                ...
                ${(residentStatus !== 'assistance' && residentStatus !== 'emergency' && !(residentStatus === 'evacuated' && occupantsEvacuated === totalOccupants))
