      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="bg-white border border-slate-200 rounded-lg px-4 py-3 flex items-center gap-2">
            <span className="text-lg font-bold text-slate-900">{reportedCount}/{totalCount}</span>
            <span className="text-sm font-medium text-slate-500">units reporting</span>
          </div>

          {isResponder && (
            <>
              <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 flex items-center gap-2"
                role="status" aria-label={`${emergencyCount} critical units`}>
                <span className="text-lg font-bold text-red-700">{emergencyCount}</span>
                <span className="text-sm font-medium text-red-600">critical</span>
              </div>
              <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 flex items-center gap-2"
                role="status" aria-label={`${assistanceCount} units need assistance`}>
                <span className="text-lg font-bold text-amber-700">{assistanceCount}</span>
                <span className="text-sm font-medium text-amber-600">need assistance</span>
              </div>
            </>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <label className="block text-sm font-bold text-slate-700 mb-1">Filter by floor</label>
            <Select
              value={selectedFloor ?? ''}
          ...
          {isResponder && (
            <div className="flex-1">
              <label className="block text-sm font-bold text-slate-700 mb-1">Filter by status</label>
              <Select
                value={statusFilter ?? ''}
  ...
  Colour legend
        ...
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-bold pointer-events-none select-none">
          {[
            ...
            { colour: '#F59E0B', label: 'Needs assistance', textColour: '#1C1917' },
            { colour: '#DC2626', label: 'Critical', textColour: '#fff' },
            { colour: '#2563EB', label: 'Pet rescue', textColour: '#fff' },
            ...(isResponder ? [
              { colour: '#EA580C', label: 'In progress', textColour: '#fff' },
          ...
          {visibleUnits.length === 0 ? (
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
              {visibleUnits.map(unit => {
                ...
                const cardContent = (
                  <>
                    <p className="font-bold text-sm leading-tight">{unit.unitNumber}</p>
                    {unit.report && unit.report.totalOccupants > 0 && (
                      <p className="text-xs font-medium opacity-90 mt-0.5">
                        {unit.report.occupantsEvacuated}/{unit.report.totalOccupants}
                      </p>
                    )}
                    {!unit.report && (
                ...
                const cardClass = "aspect-square rounded-lg p-2 flex flex-col justify-center items-center text-center transition-transform"
                ...
                return unit.report || isResponder ? (
                  <Link
                    ...
                    aria-label={`Unit ${unit.unitNumber}: ${label}${unit.report && unit.report.totalOccupants > 0 ? `, ${unit.report.occupantsEvacuated} of ${unit.report.totalOccupants} evacuated` : ''}`}
                  >
                    {cardContent}
                  </Link>
                ) : (
